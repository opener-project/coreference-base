...
class KafDocument(BaseDocument):
    ...
    def store(self, graph, encoding, language=None, version=None, linguistic_parsers=(),
        ...
        previous_kaf = None
        try:
            previous_kaf = graph.graph_properties["kaf"]
        except:
            pass

        if previous_kaf:
            kaf_document = previous_kaf
            kaf_id = graph.vertex_properties["kaf_id"]
            form = graph.vertex_properties['form']
            for coref_index, entity in enumerate(BaseGraphBuilder.extract_entities(graph), 1):
                references = [([kaf_id[word].split("#")[0] for word in BaseGraphBuilder.get_chunk_words(mention)], form[mention])
