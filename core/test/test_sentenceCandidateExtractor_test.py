...
class TestSentenceCandidateExtractor(TestCase):
    ...
    def test_validate_np_node_without_filter(self):
        ...
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", True, None, "NP")
    ...
    def test_validate_pronouns_node_without_filter(self):
        ...
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", None, "PRP")
        self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
    ...
    def test_validate_ne_node_without_filter(self):
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", None, "VP")
        self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                         "VP candidate is fetched")
        #"PERSON", "NORP", "FACILITY", "ORGANIZATION", "GPE", "LOCATION", "PRODUCT", "EVENT", "WORK OF ART","LAW",
        # "LANGUAGE", "DATE", "TIME"

        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "PERSON", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "PERSON chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "NORP", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "NORP chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "FACILITY",
                                                 "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "FACILITY chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "GPE", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "GPE chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "LOCATION",
                                                 "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "LOCATION chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "PRODUCT",
                                                 "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "PRODUCT chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "EVENT", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "EVENT chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "WORK OF ART",
                                                 "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "WORK OF ART chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "LAW", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "LAW chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "LANGUAGE",
                                                 "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "LANGUAGE chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "DATE", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
                        "DATE chunk candidate is not fetched")
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", "true", "TIME", "VP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node, filter_candidates=False),
    ...
    def test_validate_node_filter_stopwords(self):
        ...
        for form in STOPWORDS:
            test_node = self.graph_builder.add_constituent(form, self.test_graph, "true", "mention", "true", "PERSON", "VP")
        ...
        for form in STOPWORDS:
            test_node = self.graph_builder.add_constituent(form, self.test_graph, "true", "mention", "true", None, "NP")
            self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node),
                             "NP candidate with {0} is fetched".format(form))

    def test_validate_node_filter_larger_mention(self):
        head_node = self.graph_builder.add_constituent("an apple", self.test_graph, True, "mention", "no_head", "PERSON",
                                                 "VP")
        no_head_node = self.graph_builder.add_constituent("an apple", self.test_graph, False, "mention", "head", "PERSON",
                                                    "VP")
        parent_node = self.graph_builder.add_constituent("an apple", self.test_graph, True, "mention", "parent", "PERSON",
                                                   "VP")
        self.graph_builder.syntax_tree_link(parent=parent_node, child=head_node)
        self.graph_builder.syntax_tree_link(parent=parent_node, child=no_head_node)
    ...
    def test_validate_node_filter_invalid_ner(self):
        ...
        test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", True, "PERSON", "NP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node),
                        "PERSON chunk candidate is not fetched")
        for ner in INVALIDS_NER:
            test_node = self.graph_builder.add_constituent("an apple", self.test_graph, "true", "mention", True, ner, "NP")
            self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node),
    ...
    def test_validate_node_filter_quantifier_or_partitive_expressions(self):
        test_node = self.graph_builder.add_constituent("apples", self.test_graph, "true", "mention", True, None, "NP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node),
                        "NP candidate is not fetched")

        test_node = self.graph_builder.add_constituent("million of apples", self.test_graph, True, "mention",
                                                 "millions of apples", None, "NP")
        self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node),
                         "partitive candidate is fetched")

        test_node = self.graph_builder.add_constituent("any apples", self.test_graph, True, "mention", "millions of apples",
                                                 None, "NP")
        self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node),
    ...
    def test_validate_node_filter_nationality(self):
        test_node = self.graph_builder.add_constituent("apples", self.test_graph, True, "mention", "apples", None, "NP")
        self.assertTrue(self.candidate_extractor.validate_node(mention_candidate=test_node),
                        "NP candidate is not fetched")

        test_node = self.graph_builder.add_constituent("American", self.test_graph, True, "mention", "millions of apples",
                                                 None, "NP")
        self.assertFalse(self.candidate_extractor.validate_node(mention_candidate=test_node),
    ...
    def test_skip_root(self):
        ...
        s_chunk = self.graph_builder.add_constituent("the way of samurai", self.test_graph, True, "s", "the way of samurai",
                                               None, "S")
        root_chunk = self.graph_builder.add_constituent("dummyRoot", self.test_graph, True, "dummyroot", "dummyRoot", None,
                                                  "ROOT")
        plain_chunk = self.graph_builder.add_constituent("apples", self.test_graph, True, "mention", "apples", None, "NP")
        self.graph_builder.syntax_tree_link(root_chunk, s_chunk)
        self.graph_builder.syntax_tree_link(stanford_sentence_root, root_chunk)

        self.graph_builder.syntax_tree_link(conll_sentence_root, s_chunk)
        self.graph_builder.syntax_tree_link(s_chunk, plain_chunk)
    ...
    def test_order_constituent_simple(self):
        ...
        s_chunk = self.graph_builder.add_constituent("He played a song", self.test_graph, False,
                                               "S He played a song", "he played a song", None, "S")

        he_NP_chunk = self.graph_builder.add_constituent("He", self.test_graph, True,
                                                   "NP he", "he", None, "NP")
        ...
        played_VP_chunk = self.graph_builder.add_constituent("played", self.test_graph, True,
        ...
        a_new_song_NP_chunk = self.graph_builder.add_constituent("a new song", self.test_graph, False,
        ...
        self.graph_builder.syntax_tree_link(stanford_sentence_root, s_chunk)

        self.graph_builder.syntax_tree_link(he_NP_chunk, he_PRP_word)
        self.graph_builder.syntax_tree_link(s_chunk, he_NP_chunk)
        self.graph_builder.set_head(played_VBD_word)

        self.graph_builder.syntax_tree_link(played_VP_chunk, played_VBD_word)
        self.graph_builder.syntax_tree_link(s_chunk, played_VP_chunk)

        self.graph_builder.syntax_tree_link(s_chunk, a_new_song_NP_chunk)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, a_DET_word)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, new_JJ_word)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, song_NN_word)
        self.graph_builder.set_head(song_NN_word)

        self.graph_builder.syntax_tree_link(s_chunk, point_word)
    ...
    def test_order_constituent_double(self):
        ...
        stanford_sentence_root = self.graph_builder.add_sentence(self.test_graph, 0, "DummyRoot", "SentenceTest", 1)
        s_chunk = self.graph_builder.add_constituent("John is a musician", self.test_graph, False,
                                               "S Jhon is a musician", "jhon is a musician", None, "S")
        #(NP (NNP John))
        john_NP_chunk = self.graph_builder.add_constituent("John", self.test_graph, True,
        ...
        #(VP (VBZ is)
        is_VP_chunk = self.graph_builder.add_constituent("is", self.test_graph, True,
                                                   "VP is", "is", None, "VP")
        is_VBZ_word = self.graph_builder.add_word("is", self.test_graph, "word_1",
                                                  "VBZ is", "be", "O", "VBZ", stanford_sentence_root)
        #(NP (DT a) (NN musician)))
        a_musician_NP_chunk = self.graph_builder.add_constituent("a musician", self.test_graph, False,
        ...
        self.graph_builder.syntax_tree_link(john_NP_chunk, john_NNP_word)
        self.graph_builder.syntax_tree_link(s_chunk, john_NP_chunk)
        self.graph_builder.set_head(john_NNP_word)

        self.graph_builder.syntax_tree_link(is_VP_chunk, is_VBZ_word)
        self.graph_builder.syntax_tree_link(s_chunk, is_VP_chunk)
        self.graph_builder.set_head(is_VBZ_word)

        self.graph_builder.syntax_tree_link(s_chunk, a_musician_NP_chunk)
        self.graph_builder.syntax_tree_link(a_musician_NP_chunk, a_DET_word)
        self.graph_builder.syntax_tree_link(a_musician_NP_chunk, musician_NN_word)
    ...
    def test_order_constituent_full(self):
        ...
        self.graph_builder.syntax_tree_link(s_chunk, a_musician_NP_chunk)
        self.graph_builder.syntax_tree_link(a_musician_NP_chunk, a_DT_word)
        self.graph_builder.syntax_tree_link(a_musician_NP_chunk, musician_NN_word)
        self.graph_builder.set_head(musician_NN_word)
        ...
        self.graph_builder.syntax_tree_link(s_chunk, a_new_song_NP_chunk)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, a_DT_word)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, new_JJ_word)
        self.graph_builder.syntax_tree_link(a_new_song_NP_chunk, song_NN_word)
        self.graph_builder.set_head(song_NN_word)
        ...
        # ROOT (S
        stanford_sentence_root = self.graph_builder.add_sentence(self.test_graph, 0, "DummyRoot", "SentenceTest", 1)
        s_chunk = self.graph_builder.add_constituent("A girl was listening to the song", self.test_graph, False,
                                               "S A girl was listening to the song", "A girl was listening to the song",
                                               None, "S")
        # (NP (DT A) (NN girl))
        a_girl_NP_chunk = self.graph_builder.add_constituent("a girl", self.test_graph, False,
        ...
        was_listening_VP_chunk = self.graph_builder.add_constituent("was listening", self.test_graph, True,
        ...
        to_the_song_PP_chunk = self.graph_builder.add_constituent("to the song", self.test_graph, False,
        ...
        the_song_NP_chunk = self.graph_builder.add_constituent("the song", self.test_graph, False,
        ...
        self.graph_builder.syntax_tree_link(a_girl_NP_chunk, a_DT_word)
        self.graph_builder.syntax_tree_link(a_girl_NP_chunk, girl_NN_word)
        self.graph_builder.syntax_tree_link(s_chunk, a_girl_NP_chunk)
        self.graph_builder.set_head(girl_NN_word)

        self.graph_builder.syntax_tree_link(was_listening_VP_chunk, was_VBD_word)
        self.graph_builder.syntax_tree_link(was_listening_VP_chunk, listening_VBG_word)
        self.graph_builder.syntax_tree_link(s_chunk, was_listening_VP_chunk)
        self.graph_builder.set_head(listening_VBG_word)

        self.graph_builder.syntax_tree_link(the_song_NP_chunk, the_DT_word)
        self.graph_builder.syntax_tree_link(the_song_NP_chunk, song_NN_word)
        self.graph_builder.syntax_tree_link(to_the_song_PP_chunk, the_song_NP_chunk)
        self.graph_builder.set_head(song_NN_word)
        self.graph_builder.syntax_tree_link(to_the_song_PP_chunk, to_TO_word)
        self.graph_builder.syntax_tree_link(s_chunk, to_the_song_PP_chunk)
        ...
        #(ROOT (S
        stanford_sentence_root = self.graph_builder.add_sentence(self.test_graph, 0, "DummyRoot", "SentenceTest", 1)
        s_chunk = self.graph_builder.add_constituent('"It is my favorite", John said to her.', self.test_graph, False,
                                               'S "It is my favorite", John said to her.',
                                               '"It is my favorite", John said to her.', None, "S")
        # (S
        inner_s_chunk = self.graph_builder.add_constituent('"It is my favorite"', self.test_graph, False,
                                                     'S "It is my favorite"',
        ...
        open_word = self.graph_builder.add_word("``", self.test_graph, "word_4",
                                                "`` ``", "``", "O", ",", stanford_sentence_root)
        # (NP (PRP It))
        it_NP_chunk = self.graph_builder.add_constituent("It", self.test_graph, True,
                                                   "NP It", "it", None, "NP")
        ...
        my_favorite_NP_chunk = self.graph_builder.add_constituent("my favorite", self.test_graph, False,
        ...
        john_NNP_word = self.graph_builder.add_word("John", self.test_graph, "word_O",
                                                    "NNP John", "john", "PERSON", "NNP", stanford_sentence_root)
        # (VP (VBD said)
        said_VP_chunk = self.graph_builder.add_constituent("said", self.test_graph, True,
        ...
        # (PP (TO to) (NP (PRP her))))
        to_her_PP_chunk = self.graph_builder.add_constituent("to her", self.test_graph, False,
                                                       "PP to her", "to the song", None, "PP")
        to_TO_word = self.graph_builder.add_word("to", self.test_graph, "word_2",
                                                 "TO to", "to", "O", "TO", stanford_sentence_root)
        her_NP_chunk = self.graph_builder.add_constituent("her", self.test_graph, False,
                                                    "NP her", "her", None, "NP")
        ...
        self.graph_builder.syntax_tree_link(inner_s_chunk, open_word)

        self.graph_builder.syntax_tree_link(it_NP_chunk, it_PRP_word)
        self.graph_builder.syntax_tree_link(inner_s_chunk, it_NP_chunk)
        self.graph_builder.set_head(it_PRP_word)

        self.graph_builder.syntax_tree_link(is_VBZ_word, is_VP_chunk)
        self.graph_builder.syntax_tree_link(inner_s_chunk, is_VBZ_word)
        self.graph_builder.set_head(is_VBZ_word)

        self.graph_builder.syntax_tree_link(my_favorite_NP_chunk, my_PRP_word)
        self.graph_builder.syntax_tree_link(my_favorite_NP_chunk, favorite_JJ_word)
        self.graph_builder.syntax_tree_link(inner_s_chunk, my_favorite_NP_chunk)
        self.graph_builder.set_head(favorite_JJ_word)

        self.graph_builder.syntax_tree_link(inner_s_chunk, close_word)

        self.graph_builder.syntax_tree_link(s_chunk, inner_s_chunk)

        self.graph_builder.syntax_tree_link(s_chunk, coma_word)

        self.graph_builder.syntax_tree_link(john_NP_chunk, john_NNP_word)
        self.graph_builder.syntax_tree_link(s_chunk, john_NP_chunk)
        self.graph_builder.set_head(john_NNP_word)

        self.graph_builder.syntax_tree_link(said_VP_chunk, said_VBD_word)
        self.graph_builder.syntax_tree_link(s_chunk, said_VP_chunk)
        self.graph_builder.set_head(said_VBD_word)

        self.graph_builder.syntax_tree_link(her_NP_chunk, her_PRP_word)
        self.graph_builder.syntax_tree_link(to_her_PP_chunk, her_NP_chunk)
        self.graph_builder.set_head(her_PRP_word)
        self.graph_builder.syntax_tree_link(to_her_PP_chunk, to_TO_word)
        self.graph_builder.set_head(her_NP_chunk)
        self.graph_builder.syntax_tree_link(s_chunk, to_her_PP_chunk)
