...
class OntonotesGraphBuilder(BaseGraphBuilder):
    ...
    def parse_word(self, word_leaf, sentence_root_node):
        ...
        word_node = self.add_word(form=form, wid=word_id, label=label, lemma=lemma, ner=ner, pos=pos, head=False,
                                  sentence_root_node=sentence_root_node)
    ...
    def parse_syntax(self, sentence, sentence_root_node):
        ...
        def iterate( chunk, parent):
            ...
            self.syntax_tree_link(parent, new_node)
            return end
        # End Iteration
